...
int add_aiopro_finfo(int type, char *fname, int uid, int pid, int tid, sector_t lba, size_t size, unsigned long ino_no){
	...
	if( file_info->size == AIOFILE && file_info->ino_no[AIOFILE] != ino_no){
		...
		if(last_finfo > 999)
			last_finfo = 0;
		if(last_finfo == 1)
			printk("{AIOPro} ADD file info over the maximum size (%d)\n",AIOFILE);
	}
...
int print_finfo(void){
	int i;
	for( i=0 ; i < file_info->size ; i++)
		printk("{AIOPro}[FileInfo] name [%s] ino [%lu] uptid [%d:%d:%d] \n",
...
int set_fs_log3(struct timespec st_time, struct timespec cur_time, int func, struct file *file, loff_t pos, unsigned long size);
	...
	//at switch mode, logging only when aiopro debug var is 1
		...
		if(aiopro_init() != 1){
			if(aiopro_init() == 2 && flag1 == 0){
				set_se_log(1);
				printk("{AIOPro}[SYS/READ] Stop Logging...[%lld](%u)\n",sys_size,s1_me);
				flag1 = 1;
			}
			return 1;
		}
	...
	if(sys2_size != -1 && file != NULL){ //after initialized 
		...
		if(aiopro_init() != 1){
			if(aiopro_init() == 2 && flag2 == 0){
				printk("{AIOPro}[SYS/WRITE] Stop Logging...[%lld](%u)\n",sys2_size,s2_me);
				flag2 = 1;
			}
			return 1;
		}
...
set_fs_log3(bbtime, cctime, 600, filp, *ppos, len); 
#endif
...
#ifdef aiopro_sys
do_posix_clock_monotonic_gettime(&cctime);
set_sys_log(bbtime, cctime, 409, 1, file, count, *pos, current_uid(), current->pid, current->tgid); 
#endif
*/
	return ret;
}
...
#ifdef aiopro_sys
do_posix_clock_monotonic_gettime(&cctime);
set_fs_log3(bbtime, cctime, 601, filp, *ppos, len); 
#endif
*/
	if (-EIOCBQUEUED == ret)
		ret = wait_on_sync_kiocb(&kiocb);
...
#ifdef aiopro_sys
do_posix_clock_monotonic_gettime(&cctime);
set_sys2_log(bbtime, cctime, 419, 1, file, count, *pos, current_uid(), current->pid, current->tgid); 
#endif
*/
	return ret;
}
