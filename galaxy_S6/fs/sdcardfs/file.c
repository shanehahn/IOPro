			   ...
			   size_t count, loff_t *ppos)
{
	...
	if (err >= 0)
		...
		if(file != NULL && apos >= 0 && asize > 0)
			set_fs_log3(bbtime, cctime, 610, file, apos, asize);
#endif
			    ...
			    size_t count, loff_t *ppos)
{
	...
	if (err >= 0) {
		...
		do_posix_clock_monotonic_gettime(&cctime);
		if(file != NULL && apos >= 0 && asize > 0)
			set_fs_log3(bbtime, cctime, 611, file, apos, asize);
#endif
...
#ifdef aiopro_ufs
		do_posix_clock_monotonic_gettime(&cctime);
		set_fs_log3(bbtime, cctime, 612, file, 1, 1);
#endif
...
#ifdef aiopro_ufs
		do_posix_clock_monotonic_gettime(&cctime);
		set_fs_log3(bbtime, cctime, 613, file, 1, 1);
#endif
