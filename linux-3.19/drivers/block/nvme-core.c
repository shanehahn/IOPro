...
static struct timespec rq_prev;
static int rq_num=0, rq_max=0;

struct rq_start_list{
	unsigned int op_start_sec[50];
	unsigned long op_start_nsec[50];
	sector_t lba[50];
};

static struct rq_start_list rq_start;

int add_rq_start(sector_t lba, struct timespec rq_time){
	rq_start.lba[rq_num] = lba;
	rq_start.op_start_sec[rq_num] = rq_time.tv_sec;
	rq_start.op_start_nsec[rq_num] = rq_time.tv_nsec;
...
int get_rq_start(sector_t lba, struct timespec* rq_time){
	int i;	
	for(i=0 ; i< rq_max ; i++){
		if( rq_start.lba[i] == lba){
			//rq_start.lba[i] = 0;				
			rq_time->tv_sec = rq_start.op_start_sec[i];
			rq_time->tv_nsec = rq_start.op_start_nsec[i];
			return 1;
		}
	}

	//printk("{AIOPro} finding start time of UFS operation failed~!!!\n");
	rq_time->tv_sec = rq_start.op_start_sec[0];
	rq_time->tv_nsec = rq_start.op_start_nsec[0];
	return 0;
}
#endif
/////////////////////////////////////////////////////////////
						...
						nvme_completion_fn *fn)
{
...
#ifdef aiopro_scsi
	struct timespec op_end, op_start;
	struct nvme_iod *iod = cmd->ctx;
	struct request *req = iod->private;
	sector_t nlba = 0;
	unsigned long nsize = 0;
	unsigned int  nrw = 0;
	char sdname[32], fname[100];
	int ino=0;
	...
	if(req->bio != NULL){
		...
		nsize = blk_rq_bytes(req); 
		nlba = blk_rq_pos(req);
		//printk("{IOP} 5 flush RW %u Size %lu LBA %llu ", nrw, nsize, (unsigned long long) nlba); //worked -- some cases size and lba are over their limit < exception handling needed

		do_posix_clock_monotonic_gettime(&op_end);
		if(get_rq_start(nlba, &op_start) == 1)
			set_nvme_log(op_start, op_end, 920, req, nrw, nsize, nlba, sdname, fname, &ino);
		else	
			set_nvme_log(rq_prev, op_end, 920, req, nrw, nsize, nlba, sdname, fname, &ino);
...
#ifdef aiopro_scsi
// Not Verified -- 170419 Shane Hahn
/*	if(req != NULL){
		if(req->bio != NULL){
			if(rq_data_dir(req) == WRITE)
				nrw = 1; //write
			...
			nsize = blk_rq_bytes(req); 
			nlba = blk_rq_pos(req);
//			printk("{IOP} 2 flush RW %u Size %lu LBA %llu ", nrw, nsize, (unsigned long long) nlba);
		}
	}
*/
#endif
...
#ifdef aiopro_scsi
// Verified -- this is the only route to submit io to nvme device in this kernel -- 170419 Shane Hahn
/*	if(req != NULL){
		if(req->bio != NULL){
			if(rq_data_dir(req) == WRITE)
				nrw = 1; //write
			...
			nsize = blk_rq_bytes(req); 
			nlba = blk_rq_pos(req);
//			printk("{IOP} 1 iod RW %u Size %lu LBA %llu ", nrw, nsize, (unsigned long long) nlba);
		}
	}*/
#endif
#ifdef aiopro_scsi
	do_posix_clock_monotonic_gettime(&rq_prev);
	if(req != NULL){
	if(req->bio != NULL){
		nlba = blk_rq_pos(req);
		if(nlba > 0)
			add_rq_start(nlba, rq_prev);
	}}
#endif
...
// Not Verified -- 170419 Shane Hahn
	if(req != NULL){
		if(req->bio != NULL){
			...
			nsize = blk_rq_bytes(req); 
			nlba = blk_rq_pos(req);
//			printk("{IOP} 0 Qrq RW %u Size %lu LBA %llu ", nrw, nsize, (unsigned long long) nlba);
		}
	}
#endif
