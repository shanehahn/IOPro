...
//if(aiopro_init() == 1){
	...
	set_sys_log(bbtime, cctime, 400, fd, f.file, count, pos, current_uid().val, current->pid, current->tgid);
//}
#endif
...
//if(aiopro_init() == 1){
	do_posix_clock_monotonic_gettime(&cctime);
	set_sys2_log(bbtime, cctime, 410, fd, f.file, count, pos, current_uid().val, current->pid, current->tgid); 
//}
#endif
