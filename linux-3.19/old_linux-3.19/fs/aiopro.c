...
//////////////////////////////////////////////////////////////path for log                                                                                       
#ifdef aiopro_ubuntu
#define SYS_PATH "/home/shanehahn/log_sys.txt"
#define SYS2_PATH "/home/shanehahn/log_sys2.txt"
#define MM_PATH "/home/shanehahn/log_mm.txt"
#define SE_PATH "/home/shanehahn/log_se.txt"
#define FS_PATH "/home/shanehahn/log_fs.txt"
#define BIO_PATH "/home/shanehahn/log_bio.txt"
#define SCSI_PATH "/home/shanehahn/log_scsi.txt"
#define UFS_PATH "/home/shanehahn/log_ufs.txt"
#endif
	...
	unsigned long long itime;
        ...
        if(aiopro_init() != 1 && aiopro_init() != 3){
                ...
                if( size < 2)
                        ...
                        printk("{IOPro}[PRINT/MMC] Prepare for writing data to file after stopping logging [%lld]\n",size);
			scsicnt = -1;
			msleep(1500);
		}
        }
		...
		printk("{IOPro}[PRINT/MMC] Start writing data to file (%lld)\n",size);

                //check if log size was too small to store previous occured I/O log                                                                              
                err = get_scsi_err();
                if(err > 0 && err > scsiperr){
                        printk("{IOPro}[Print/MMC] logging size is over predefined maximum log size~!!! (%llu)\n", size);
                        scsiperr = err;
                }
...
#endif
		{
			printk (KERN_INFO "{IOPro}[Print/MMC] fwrite: file_open error (%s)\n", UFS_PATH);
			scsi_fo=1;
			return 1;
		}
...
//                                if( i%50000 == 0)
  //                                      printk("{IOPro}[Print/UFS] Writing logs after stopping loggging is processing (%lld/%lld)\n",i,(size-1));
                		if( i == (size-1))
                                        printk("{IOPro}[Print/MMC] Finish writing logs after stopping loggging (%lld)\n",i);
                                continue;
        	...
        	if(scsicnt == 0){
		        ...
		        befscsi = size;
                        ...
                        if( (scsicc.tv_sec - scsibb.tv_sec) > WAITV){
                                if(  (size - befscsi) < 10 ){
                                        printk("{IOPro}[Print/MMC] Waiting time is over predefined maximum time[%d]. Start writing log at file. (%lld)\n",WAITV, size);
                                        scsicnt = -1;
                		}
				else{
	                        	printk("{IOPro}[Print/MMC] Logging... (%lld)\n", size);
         			        scsicnt = 0;
		                }
                        }
                }
...
static int aiopro_scsi_thread(void *args){
	msleep(100);
	printk("{IOPro}[Kthread] Starting kernel thread for MMC\n");
	while( kthread_should_stop() != 1 && scsi_fo != 1 ){
		print_scsi_result();
	}
	printk("{IOPro}[Kthread] <<< STOP >>> kthread for MMC\n");
	return 0;
}
...
int init_aiopro_scsi(void){   ///////////////////////
	if(aiopro_scsi_task == NULL){
		aiopro_scsi_task = kthread_run(aiopro_scsi_thread,NULL,"IOPro_MMC");
		return 1;
	}
