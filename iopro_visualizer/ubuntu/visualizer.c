...
void visualize(unsigned long num_line) {
	...
	for(i = 0; i < num_line-2; i++) {
		if (!strcmp(g_func_id, "418")) { //fsync/fdatasync
			...
			if (strstr(info_file, "info_400_1.txt") && strstr(lines[i].filename, "fg_file")) {
				fprintf(stderr, "###########GOTCHA!!###########\n");
				fprintf(stderr, "#%lu ?%s?%s size:%lu offset:%lu %s %s file:%s\n",
			...
			if (!strcmp(g_func_id, "701") || !strcmp(g_func_id, "702")) { //cfq_insert_request cfq_dispatch_insert
				printf("#%lu ?%s?%s size:%lu lba:%lu %s %s file:%s fpb:%lu\n",lines[i].start_time, 
				       config.color, config.layer_name, lines[i].size, 
				       lines[i].lba, lines[i].rw, lines[i].io_flag, lines[i].filename, lines[i].offset);
				printf("#%lu\n",lines[i].end_time);
