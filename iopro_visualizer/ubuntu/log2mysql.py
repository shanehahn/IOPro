...
def main(argv):
  ...
  try:
    # MySQL DB connection
    con = mdb.connect(serveraddr, user, password)
    cur = con.cursor()
...
def createTable(cur, table):
  cur.execute('CREATE TABLE ' + table + ' (start_time_sec VARCHAR(45), \
  start_time_nsec VARCHAR(45), exec_time_nsec VARCHAR(45), uid INT(11), \
  pid INT(11), tid INT(11), IO_layer INT(11), type INT(11), func_id VARCHAR(45), \
  func_id_2 VARCHAR(45), fd INT(11), offset VARCHAR(45), size VARCHAR(45), \
  inode_num VARCHAR(45), lba VARCHAR(45), filename VARCHAR(45), \
  start_time VARCHAR(45), end_time VARCHAR(45), run_time INT(10), trun_time INT(10))')
